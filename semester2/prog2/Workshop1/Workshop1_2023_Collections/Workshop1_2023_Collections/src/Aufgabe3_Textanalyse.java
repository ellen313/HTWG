...
public class Aufgabe3_Textanalyse {
    public static void loesung() throws IOException {
        ...
        // Ihr Code: ...
        Set<String> s = new TreeSet<>(lst_Kafka);
        int count = 100;
        for (String st : s) {
        ...
        List<String> lst_Deutsch = einlesen("data/word_list_german_spell_checked.txt");
        ...
        for (String st : lst_Kafka) {
            if (!ww.contains(st)){
                if (!fqTable_falsch.containsKey(st)){
                    fqTable_falsch.put(st, 1);
                } else {
                    fqTable_falsch.put(st, fqTable_falsch.get(st) + 1);
                }
            }
        }
        print20(fqTable_falsch);
        ...
        print20(fqTable_Kafka_Harry);
    }

    private static void print20(Map<String, Integer> fqTable) {
