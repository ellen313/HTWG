...
public class Workshop2 {
    ...
    public static void teil2() {
        ...
        int count = 0;
        for (String s : myList)
            if (Objects.equals(s, myString))
                count++;
        System.out.println(WHITE + "Externe Iteration: " + count);
        ...
        System.out.println("Interne Iteration: " + nAtomic);

        // c) (2 Punkte) Mit einem Stream:
        long count3 = myList.stream()
                .filter(s -> Objects.equals(s, myString))
                .count();
        System.out.println("Streams: " + count3);
    }
    ...
    public static void teil3() {
        ...
        Map<String, List<Stadt>> mp = sLst.stream().collect(Collectors.groupingBy(Stadt::land));
        for (Stadt s : sLst)
            if (!mp.containsKey(s.land())) {
                mp.put(s.land(), new LinkedList<>());
            } else {
                mp.get(s.land()).add(s);
            }
    ...
    public static void teil4() throws IOException {
        ...
        System.out.println("\n5 a)");
        BufferedReader in = new BufferedReader(new FileReader("C:\\prog2-uebungen\\Aufgaben\\Workshop2\\Workshop2_2023\\data\\movies-top-grossing.txt"));
        long sum = in.lines().sorted().peek(x -> {
        ...
        System.out.println("\n5 c)");
        in = new BufferedReader(new FileReader("C:\\prog2-uebungen\\Aufgaben\\Workshop2\\Workshop2_2023\\data\\movies-top-grossing.txt"));
        List<Movie> myMovieList = in.lines().map(Movie::new).toList();
        in.close();
        ...
        myMovieList.stream().filter(x -> x.actors().contains(s1)).forEach(x -> System.out.println(x.title() + " " + x.year()));

        // e) (4 Punkte)
        // Wieviel verschiedene Schauspielerinnen und Schauspieler enthält die Filmdatei?
        System.out.println("\n5 e)");
        int count = (int) myMovieList.stream().flatMap(x -> x.actors().stream()).distinct().count();
        ...
        System.out.println("\n5 f)");
        Map<Integer, List<Movie>>  mp = myMovieList.stream()
                .collect(Collectors.groupingBy(Movie::year, Collectors.toList()));
        for (Movie m : mp.get(1990))
